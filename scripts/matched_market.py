...
def generate_dma_data(
    ...
    """
    dma_data = copy.deepcopy(dma_census_data)

    dataframes = {k: v for k, v in dma_data.items() if k != "CPM DMA"}

    if len(included_datasets) > 0:
        dataframes = {k: v for k, v in dataframes.items() if k in included_datasets}
    ...
    for n, df in dataframes.items():
        ...
        if "Median" not in n:
            df.columns = [
                (
                    f"{k}_{nc.lower().replace(' ','_')}"
                    if k not in [DMA_CODE, DMA_NAME]
                    else k
                )
                for k in list(df)
            ]
        ...
        base = base.merge(df, on=[DMA_CODE, DMA_NAME], how="left")
    return base
